...
import { ConfigLoader, createConfigQuery, fromMemory, withZod } from "@/index";
import { ErrorBoundary } from "../../_shared/ErrorBoundary";

interface ServerConfig { host: string; port: number; secure: boolean }

// Duck-typed Zod-compatible schema — replace with real z.object({...}) in your app
const Schema = {
  parse: (raw: unknown): ServerConfig => {
...
const loader = ConfigLoader.create<ServerConfig>({
  ...
  validate: withZod(Schema),
});

const query = createConfigQuery("validator_zod", loader);
